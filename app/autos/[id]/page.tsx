...
export default function DetalleAuto() {
  ...
  const verificarFavorito = async (userId: string) => {
    const { data } = await supabase
      ...
      .eq('usuario_id', userId).eq('auto_id', params.id).single()
    setEsFavorito(!!data)
  }
            ...
            <div style={{background: '#fff', borderRadius: '16px', padding: '24px', border: '1px solid #eee'}}>
              ...
              {usuario ? (
                <div style={{display: 'flex', flexDirection: 'column', gap: '10px'}}>
                  ...
                  {/* Botón contactar — solo verificados */}
                  {perfilVerificado ? (
                    <button
                    ...
                    <div>
                      <div style={{background: '#fffbeb', border: '1px solid #fde68a', borderRadius: '10px', padding: '12px 16px', marginBottom: '10px'}}>
                  ...
                  <button
                    ...
                    style={{width: '100%', background: esFavorito ? '#eff6ff' : '#fff', color: esFavorito ? '#2563eb' : '#333', border: `1.5px solid ${esFavorito ? '#2563eb' : '#e5e5e5'}`, padding: '14px', borderRadius: '10px', fontSize: '15px', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px'}}
                  >
                ...
                <div>
                  ...
                  <button onClick={() => router.push('/login')} className="btn-contactar" style={{width: '100%', background: '#2563eb', color: '#fff', border: 'none', padding: '14px', borderRadius: '10px', fontSize: '15px', fontWeight: '700', cursor: 'pointer'}}>
