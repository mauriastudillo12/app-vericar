...
function ChatContent() {
  ...
  // Crea una conversación nueva o devuelve la existente
  ...
  // Carga todas las conversaciones del usuario sin duplicados
  const cargarConversaciones = async (userId: string) => {
    ...
    // Enriquecer cada conversación con datos del auto y del otro usuario
    const conversacionesCompletas = await Promise.all(lista.map(async (conv: any) => {
      ...
      const { data: otroData } = await supabase
        .from('perfiles')
        .select('nombre, email')
        .eq('id', otroId)
        .single()
    ...
    // Eliminar conversaciones duplicadas — mismo auto y mismo otro usuario
    const vistas = new Set()
    const conversacionesSinDuplicados = conversacionesCompletas.filter((conv: any) => {
      const otroId = conv.comprador_id === userId ? conv.vendedor_id : conv.comprador_id
    ...
    return conversacionesSinDuplicados
  }

  // Carga mensajes y los marca como leídos
  ...
  // Abre una conversación e inicia el polling
  ...
  useEffect(() => {
    const iniciar = async () => {
      ...
      // Si viene desde detalle de auto, crear o buscar conversación
      ...
      // Abrir la conversación correspondiente o la primera de la lista
      if (lista.length > 0) {
        const aAbrir = convId
          ? lista.find((c: any) => c.id === convId) || lista[0]
          : lista[0]
  ...
  // Enviar mensaje y recargar inmediatamente
  ...
  const otroUsuario = (conv: any) => {
    return conv.otroUsuarioData?.nombre || conv.otroUsuarioData?.email || 'Usuario'
  }
          ...
          {conversacionActiva ? (
              ...
              <div style={{
                ...
                <div>
                  <div style={{fontSize: '15px', fontWeight: '700', color: '#000'}}>
                    {otroUsuario(conversacionActiva)}
                  </div>
                  <div style={{fontSize: '12px', color: '#888'}}>
                    {conversacionActiva.auto?.nombre}
                  </div>
                </div>
              </div>
                    ...
                    return (
                      <div key={msg.id} style={{
                        ...
                        <div style={{
                          ...
                          <div style={{
                            fontSize: '10px',
                            color: esMio ? 'rgba(255,255,255,0.6)' : '#aaa',
                            marginTop: '4px',
              ...
              <div style={{
                ...
                <input
                  ...
                  style={{
                    ...
                    border: '1.5px solid #e5e5e5', borderRadius: '24px',
            ...
            <div style={{flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fafafa'}}>
              <div style={{textAlign: 'center'}}>
                ...
                <p style={{fontSize: '16px', fontWeight: '600', color: '#333', marginBottom: '6px'}}>
                  Selecciona una conversación
                </p>
                <p style={{fontSize: '14px', color: '#888'}}>
                  O contacta a un vendedor desde el detalle de un auto
                </p>
              </div>
            </div>
          )}
