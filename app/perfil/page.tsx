...
export default function Perfil() {
  ...
  // Tab activa — autos, repuestos o favoritos
  ...
  useEffect(() => {
    const cargarPerfil = async () => {
      ...
      // Cargar favoritos del usuario con datos del auto
  ...
  const eliminarAuto = async (autoId: string) => {
    const confirmar = window.confirm('¿Estás seguro que quieres eliminar esta publicación? Esta acción no se puede deshacer.')
    if (!confirmar) return
  ...
  // Eliminar una publicación de repuesto
  const eliminarRepuesto = async (repuestoId: string) => {
    const confirmar = window.confirm('¿Estás seguro que quieres eliminar este repuesto? Esta acción no se puede deshacer.')
    if (!confirmar) return
        ...
        <div style={{display: 'grid', gridTemplateColumns: '300px 1fr', gap: '32px', alignItems: 'start'}}>
          ...
          <div style={{display: 'flex', flexDirection: 'column', gap: '16px'}}>
            ...
            <div style={{background: '#fff', borderRadius: '16px', padding: '32px 24px', border: '1px solid #eee', textAlign: 'center'}}>
              ...
              {/* Nombre o formulario de edición */}
              {editando ? (
                <div style={{marginBottom: '16px'}}>
                  <input
                    ...
                    style={{
                      ...
                      background: '#fafafa', color: '#000', boxSizing: 'border-box',
                  ...
                  <div style={{display: 'flex', gap: '8px'}}>
                    <button
                      ...
                      disabled={guardando}
                      style={{flex: 1, background: '#2563eb', color: '#fff', border: 'none', padding: '8px', borderRadius: '8px', fontSize: '13px', fontWeight: '700', cursor: 'pointer'}}
                    >
                      {guardando ? 'Guardando...' : 'Guardar'}
                    </button>
                    <button
                      onClick={() => { setEditando(false); setNuevoNombre(perfil?.nombre || '') }}
                      style={{flex: 1, background: '#fff', color: '#666', border: '1px solid #e5e5e5', padding: '8px', borderRadius: '8px', fontSize: '13px', cursor: 'pointer'}}
                    >
                      Cancelar
                    </button>
                  </div>
              ...
              {!editando && (
                <button
                  ...
                  style={{width: '100%', background: '#fff', border: '1.5px solid #e5e5e5', color: '#333', padding: '10px', borderRadius: '10px', fontSize: '13px', fontWeight: '600', cursor: 'pointer'}}
                >
                  Editar perfil
                </button>
              )}
            ...
            {!perfil?.verificado && (
              <div style={{background: '#fffbeb', borderRadius: '16px', padding: '20px', border: '1px solid #fde68a'}}>
                ...
                <button
                 ...
                 style={{width: '100%', background: '#000', color: '#fff', border: 'none', padding: '10px', borderRadius: '8px', fontSize: '13px', fontWeight: '700', cursor: 'pointer'}}
                  >
                   Verificar ahora
             </button>
              </div>
          ...
          <div>
            ...
            <div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '12px'}}>
              <div style={{display: 'flex', gap: '8px'}}>
                ...
                {/* Tab Autos */}
                <div className="tab-btn" onClick={() => setTabActiva('autos')} style={{
                  padding: '8px 20px', borderRadius: '8px', fontSize: '14px', fontWeight: '700',
                  background: tabActiva === 'autos' ? '#2563eb' : '#fff',
                  color: tabActiva === 'autos' ? '#fff' : '#888',
                  border: `1.5px solid ${tabActiva === 'autos' ? '#2563eb' : '#e5e5e5'}`,
                }}>
                  Autos {autos.length > 0 && `(${autos.length})`}
                </div>

                {/* Tab Repuestos */}
                <div className="tab-btn" onClick={() => setTabActiva('repuestos')} style={{
                  padding: '8px 20px', borderRadius: '8px', fontSize: '14px', fontWeight: '700',
                  background: tabActiva === 'repuestos' ? '#2563eb' : '#fff',
                  color: tabActiva === 'repuestos' ? '#fff' : '#888',
                  border: `1.5px solid ${tabActiva === 'repuestos' ? '#2563eb' : '#e5e5e5'}`,
                }}>
                  Repuestos {repuestos.length > 0 && `(${repuestos.length})`}
                </div>

                {/* Tab Favoritos */}
                <div className="tab-btn" onClick={() => setTabActiva('favoritos')} style={{
                  padding: '8px 20px', borderRadius: '8px', fontSize: '14px', fontWeight: '700',
                  background: tabActiva === 'favoritos' ? '#2563eb' : '#fff',
                  color: tabActiva === 'favoritos' ? '#fff' : '#888',
                  border: `1.5px solid ${tabActiva === 'favoritos' ? '#2563eb' : '#e5e5e5'}`,
                }}>
                  Favoritos {favoritos.length > 0 && `(${favoritos.length})`}
                </div>
              </div>

              {/* Botón publicar según tab activa */}
              {tabActiva === 'autos' && (
            ...
            {tabActiva === 'autos' && (
              autos.length === 0 ? (
                ...
                <div style={{display: 'flex', flexDirection: 'column', gap: '16px'}}>
                  {autos.map((auto) => (
                    <div key={auto.id} className="pub-card" style={{background: '#fff', borderRadius: '16px', border: '1px solid #eee', overflow: 'hidden', display: 'flex'}}>
                      <div style={{width: '160px', minWidth: '160px', height: '120px', background: 'linear-gradient(135deg, #e8e8e8 0%, #d5d5d5 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden'}}>
                        {(() => {
            ...
            {tabActiva === 'repuestos' && (
              repuestos.length === 0 ? (
                ...
                <div style={{display: 'flex', flexDirection: 'column', gap: '16px'}}>
                  {repuestos.map((rep) => (
                    <div key={rep.id} className="pub-card" style={{background: '#fff', borderRadius: '16px', border: '1px solid #eee', overflow: 'hidden', display: 'flex'}}>
                      <div style={{width: '160px', minWidth: '160px', height: '120px', background: 'linear-gradient(135deg, #e8e8e8 0%, #d5d5d5 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden'}}>
                        {(() => {
              ...
              favoritos.length === 0 ? (
                ...
                <div style={{display: 'flex', flexDirection: 'column', gap: '16px'}}>
                  {favoritos.map((fav) => {
                    ...
                    if (!auto) return null
                    return (
                      <div key={fav.id} className="pub-card" style={{background: '#fff', borderRadius: '16px', border: '1px solid #eee', overflow: 'hidden', display: 'flex'}}>
                        <div style={{width: '160px', minWidth: '160px', height: '120px', background: 'linear-gradient(135deg, #e8e8e8 0%, #d5d5d5 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden'}}>
                          {(() => {
                            const fotosArray = typeof auto.fotos === 'string' && auto.fotos ? JSON.parse(auto.fotos) : auto.fotos
                            return fotosArray && fotosArray.length > 0
